...
class RobotTargetMessage(SensorMessage):
    """
    Convenience class for sending target color to vision

    To send a message to the vision, just do the following:

    message = RobotTargetMessage()
    message.color = 'red'
    channel_to_rio.send_to(message.encode_message())
    """
    def __init__(self, sender="robot", message="target"):
        super(RobotTargetMessage,self).__init__(sender, message)
        self.color = 'red'
...
class LidarWallMessage(SensorMessage):
    ...
    """
    def __init__(self, sender="lidar", message="wall"):
        super(LidarWallMessage,self).__init__(sender, message)
        self.orientation = 0
        self.status = 'ok'
        self.heading = 0
        self.range = 0
...
class RobotMessage(object):
    ...
    the robot.      Just decodes the json in to a dictionary
    and then provides access to contents via attributes.
    """
